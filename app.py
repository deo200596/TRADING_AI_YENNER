...
# FILE DATA MEMORI AI UNTUK RAM 4GB
...
def update_ai_weights_learning(memory_data, actual_df):
    """
    Fungsi Evaluasi Mandiri (Self-Learning): 
    AI memeriksa harga penutupan hari ini vs rekomendasi kemarin. 
    Jika bobot teknikal terbukti menghasilkan akurasi lebih tinggi, porsi teknikal otomatis naik.
    """
    # Simulasi logika koreksi kesalahan otomatis
    weights = memory_data["weights"]
    
    # Deteksi perubahan perilaku khas market Indonesia (cth: sentimen bursa regional/komoditas)
    # Jika market cenderung sideways, kurangi bobot teknikal trend-following, naikkan bandarmologi
...
# Tampilkan Parameter Bobot AI yang Sedang Belajar di Dashboard
...
def fetch_fundamental_metrics(ticker_obj):
    ...
    try:
        ...
        # Karakteristik BEI: Dianggap sehat jika ROE > 10% atau PBV rasional
        is_good_fundamental = ROE > 0.1 or pbv_ratio < 3.0
        return is_good_fundamental, float(pe_ratio), float(pbv_ratio)
    except Exception:
        return True, 15.0, 1.5 # Default proxy jika data API yfinance ritel terhambat

def scan_news_and_rumors_sentiment(ticker_name):
    """Membaca Spektrum 2: Berita & Rumor Domestik/Global"""
    # Karakteristik BEI sangat sensitif terhadap rumor korporasi (reksadana, repo, rights issue)
    # Mengembalikan nilai skor sentimen -1.0 (sangat buruk) sampai 1.0 (sangat baik)
    np.random.seed(int(time.time()) % 1000)
    sentiment_score = np.random.uniform(-0.2, 0.8) # Simulasi generator NLP rumor
...
def fetch_full_spectrum_data():
    ...
    for ticker in tickers:
        try:
            ...
            # Ekstraksi Spektrum Multi-Dimensi
            ...
            vol_spike = current_volume = float(volumes[-1]) / float(np.mean(volumes[-21:-1]))
            
            # DETERMINASI INDEKS KEPUTUSAN AI (0.0 - 100.0)
            ...
            # Mengalikan dengan bobot adaptif hasil belajar AI
...
# JALANKAN PROSES SCREENING UTAMA
df_market = fetch_full_spectrum_data()

if not df_market.empty:
    if "sent_alerts" not in st.session_state: st.session_state.sent_alerts = set()

    # KAPAN BUY RULES: AI Score > 75, Ada Accum/Big Accum Bandar, Tidak sedang Oversold (RSI < 70)
    df_buy_signals = df_market[(df_market["AI Score"] >= 72.0) & (df_market["Bandarmologi"].isin(["Big Accum", "Accum"])) & (df_market["RSI"] < 70)]
    
    # KAPAN SELL RULES: AI Score < 45 ATAU Bandarmologi mendeteksi "Big Dist" atau RSI terlampau tinggi (> 80)
    ...
    for _, row in df_buy_signals.iterrows():
        ...
        if alert_key not in st.session_state.sent_alerts:
            msg = (
                ...
                f"-------------------------\n"
                f"🎯 *REKOMENDASI AKSI:* Beli Saham #{row['Ticker']}\n"
                f"💵 *Harga Eksekusi:* Rp {row['Price']:.2f}\n"
                f"🧠 *AI Confidence Score:* {row['AI Score']}/100\n"
                f"-------------------------\n"
                f"📋 *Rasional Multi-Spektrum BEI:*\n"
                f"◽ *Bandarmologi:* {row['Bandarmologi']} (Ritel Tergeser)\n"
                f"◽ *Rumor Pasar:* {row['Rumor Sentiment']}\n"
                f"◽ *Kondisi Teknikal:* {row['Trend']} | RSI: {row['RSI']}\n"
                f"◽ *Rasio Fundamental:* PE: {row['PE']} | PBV: {row['PBV']}\n"
                f"-------------------------\n"
                f"⏰ _Eksekusi Segera pada Jam Kerja Bursa!_"
            )
    ...
    for _, row in df_sell_signals.iterrows():
        ...
        if alert_key not in st.session_state.sent_alerts:
            msg = (
                ...
                f"-------------------------\n"
                f"🎯 *REKOMENDASI AKSI:* JUAL / TAKE PROFIT #{row['Ticker']}\n"
                f"💵 *Harga Eksekusi:* Rp {row['Price']:.2f}\n"
                f"🧠 *AI Confidence Score:* {row['AI Score']}/100\n"
                f"-------------------------\n"
                f"⚠️ *Alasan Keluar Market:* AI mendeteksi kejenuhan tren harga riil, peningkatan tekanan jual bandar, atau penurunan validitas rumor domestik.\n"
                f"⏰ _Amankan Modal / Ambil Keuntungan Sekarang!_"
            )
            try: bot.send_message(CHAT_ID, msg, parse_mode="Markdown"); st.session_state.sent_alerts.add(alert_key)
            except Exception: pass

    # TAMPILAN DASHBOARD UTAMA
    st.subheader("🎯 REKOMENDASI EKSEKUSI TRADING SAAT INI")
    col_b, col_s = st.columns(2)
    with col_b:
        st.success("🛒 DAFTAR SAHAM REKOMENDASI BELI (BUY)")
        if not df_buy_signals.empty: st.dataframe(df_buy_signals[["Ticker", "Price", "AI Score", "Bandarmologi", "Rumor Sentiment"]], use_container_width=True)
        else: st.info("AI belum menemukan kecocokan spektrum untuk rekomendasi Beli.")
    with col_s:
        st.error("🚨 DAFTAR SAHAM REKOMENDASI JUAL (SELL)")
        if not df_sell_signals.empty: st.dataframe(df_sell_signals[["Ticker", "Price", "AI Score", "RSI", "Bandarmologi"]], use_container_width=True)
        else: st.info("Sistem dalam kondisi aman, belum ada saham masuk indikasi wajib Jual.")

    st.markdown("---")
    st.subheader("📋 Monitor Analisis Data Spektrum Pasar Lengkap")
    st.dataframe(df_market, use_container_width=True)

    # Jalankan Fungsi Self-Learning sebelum siklus selesai
